...
void can_init(void)
{																	// 初始化CAN滤波器
	  HAL_CAN_Start(&hcan1);// 启动CAN控制器
	  HAL_CAN_ActivateNotification(&hcan1, CAN_IT_RX_FIFO0_MSG_PENDING); 	// 使能CAN
    // 控制器接收中断
    HAL_Delay(2000);
}
/********** 1-外设硬件初始化函数 **********/
...
void CAN_RXCALLBACK(void)
{
	...
	{
		// 一帧数据接收完成，置位帧标志位
		for(uint8_t i=can.CAN_RxMsg.DLC; i < 8; i++) { can.rxData[i] = 0; } can.rxFrameFlag = true;
	}
}
